...
use crate::normalizer::num2vi::{n2w, n2w_single, n2w_decimal};

static RE_NUMBER: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?<!\d)(?P<neg>[-–—])?(\d+(?:,\d+|(?:\.\d{3})+(?!\d)|\.\d+|(?:\s\d{3})+(?!\d))?)(?!\d)").unwrap()
});

pub static RE_MULTIPLY: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)(\b\d{1,15}(?:\s*(?![x×]\b)[a-zA-Zμµ²³°]+[0-9]*)*)(?:\s*[x×]\s*\d{1,15}(?:\s*(?![x×]\b)[a-zA-Zμµ²³°]+[0-9]*)*)+").unwrap()
});
...
fn normalize_dot_sep(number: &str) -> String {
    if RE_DOT_SEP.is_match(number).unwrap_or(false) && number.chars().filter(|&c: &char| c == '.').count() > 0 {
         if let Ok(Some(m)) = RE_DOT_SEP.find(number) {
             if m.as_str() == number {
...
pub fn num_to_words(number: &str, negative: bool) -> String {
    if number.contains('.') {
        let is_thousands = if let Ok(Some(m)) = RE_DOT_SEP.find(number) {
            m.as_str() == number
        } else {
            false
        };
...
pub fn expand_multiply_number(text: &str) -> String {
    let re = Regex::new(r"(?i)(?<=\d|[a-zA-Zμµ²³°])\s*[x×]\s*(?=\d)").unwrap();
    re.replace_all(text, " nhân ").to_string()
}
...
pub fn normalize_number_vi(text: &str) -> String {
    ...
    result = RE_NUMBER.replace_all(&temp_result, |caps: &Captures| {
