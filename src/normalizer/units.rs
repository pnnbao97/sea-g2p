...
use crate::normalizer::num2vi::{n2w, n2w_decimal};
use crate::normalizer::resources::{MEASUREMENT_KEY_VI, CURRENCY_KEY, CURRENCY_SYMBOL_MAP};
...
pub fn fix_english_style_numbers(text: &str) -> String {
    let re = Regex::new(r"(?i)\b\d{1,3}(?:,\d{3})+(?:\.\d+)?\b").unwrap();
...
pub fn expand_power_of_ten(text: &str) -> String {
    let re = Regex::new(r"(?i)\b(\d+(?:[.,]\d+)?)\s*[x*×]\s*10\^([-+]?\d+)\b").unwrap();
    re.replace_all(text, |caps: &Captures| {
...
pub fn expand_scientific_notation(text: &str) -> String {
    let re = Regex::new(r"(?i)\b(\d+(?:[.,]\d+)?e[+-]?\d+)\b").unwrap();
    re.replace_all(text, |caps: &Captures| {
