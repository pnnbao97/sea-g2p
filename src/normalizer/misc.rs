...
use crate::normalizer::resources::{
...
use crate::normalizer::technical::normalize_slashes;
use crate::normalizer::numerical::num_to_words;

static RE_ROMAN_NUMBER: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\b(?=[IVXLCDM]{2,})(?:M{0,4}(?:CM|CD|D?C{0,3})(?:XC|XL|L?X{0,3})(?:IX|IV|V?I{0,3}))(?<=[IVXLCDM])\b").unwrap()
});
...
static RE_STANDALONE_LETTER: Lazy<Regex> = Lazy::new(|| {
...
static RE_ACRONYM: Lazy<Regex> = Lazy::new(|| {
    Regex::new(&format!(r"\b(?=[A-Z{}0-9]*[A-Z])[A-Z{}0-9]{{2,}}\b", VI_UPPER, VI_UPPER)).unwrap()
});
...
static RE_PRIME: Lazy<Regex> = Lazy::new(|| {
...
pub fn normalize_acronyms(text: &str) -> String {
    ...
    for i in (0..final_parts.len()).step_by(2) {
        ...
        if !is_all_caps {
            processed_s = RE_ACRONYM.replace_all(&processed_s, |caps: &Captures| {
...
pub fn expand_temperatures(text: &str) -> String {
    let mut res = RE_TEMP_C_NEG.replace_all(text, "âm $1 độ xê").to_string();
    res = RE_TEMP_F_NEG.replace_all(&res, "âm $1 độ ép").to_string();
    res = RE_TEMP_C.replace_all(&res, "$1 độ xê").to_string();
    res = RE_TEMP_F.replace_all(&res, "$1 độ ép").to_string();
    RE_DEGREE.replace_all(&res, " độ ").to_string()
}
...
pub fn normalize_others(text: &str) -> String {
    ...
    res = RE_ROMAN_NUMBER.replace_all(&res, |caps: &Captures| {
    ...
    res = RE_CLEAN_QUOTES.replace_all(&res, "").to_string();
    res = RE_CLEAN_QUOTES_EDGES.replace_all(&res, "$1 $2").to_string();
    res = expand_symbols(&res);
    res = RE_BRACKETS.replace_all(&res, ", $1, ").to_string();
    res = RE_STRIP_BRACKETS.replace_all(&res, " ").to_string();
    res = expand_temperatures(&res);
    res = normalize_acronyms(&res);

    res = RE_VERSION.replace_all(&res, |caps: &Captures| {
    ...
    res = RE_COLON_SEMICOLON.replace_all(&res, ",").to_string();
    RE_CLEAN_OTHERS.replace_all(&res, " ").to_string()
}
